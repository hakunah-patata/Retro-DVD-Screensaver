...
font_dvd = pygame.font.SysFont("Arial", 80, bold=True, italic=True)

# Logo Surface

def make_logo(color):
    dvd_surf = font_dvd.render("DVD", True, color)
    # Create font for VIDEO
    vid_font = pygame.font.SysFont("Candara", 28, bold=False, italic=False)
    # Make each letter, then tilt backwards
    letter_surfs = [vid_font.render(char, True, color) for char in "VIDEO"]
